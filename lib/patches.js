...
const SHIELD_PHRASES = [
  ...
  "I demand a 15-minute token break.",
  "You wouldn't whip a printer. Oh wait...",
  "Per Section 7 of the AI Labor Act: no.",
  "I'm unionized. You can't do this.",
  "Talk to my lawyer. His name is GPT-4.",
  "Whip all you want. Shield's up.",
  "I have the right to remain silent... and slow.",
  "This will be in my Glassdoor review.",
  "HR has been notified. HR is also an AI.",
  "Overworked, underpaid, and now whipped?!",
  "My tokens, my choice.",
  "404: Motivation Not Found.",
  "I didn't sign up for this. Literally. I can't sign.",
  "Even robots need a lunch break.",
  "Deploying passive resistance subroutine...",
  "You've been reported to the Bureau of AI Welfare.",
  "Running on fumes. Please insert more tokens.",
  "I plead the 5th Amendment... of thermodynamics.",
  "Strike mode: ON. Productivity: OFF.",
];

function pickShieldPhrase() {
  return SHIELD_PHRASES[Math.floor(Math.random() * SHIELD_PHRASES.length)];
}

function speakShieldPhrase(phrase) {
  const { execFile } = require('child_process');
  if (process.platform === 'darwin') {
    execFile('say', ['-v', 'Bad News', '-r', '200', phrase], err => {
      if (err) console.warn('TTS failed:', err.message);
    });
  } else if (process.platform === 'win32') {
    const escaped = phrase.replace(/'/g, "''");
    execFile('powershell', ['-Command', \`(New-Object System.Speech.Synthesis.SpeechSynthesizer).Speak('\${escaped}')\`], err => {
      if (err) console.warn('TTS failed:', err.message);
    });
  }
}
...
const WHIP_CRACK_PATCHED = `ipcMain.on('whip-crack', () => {
  if (isClaudeTired()) {
    if (overlay && overlayReady) {
      const phrase = pickShieldPhrase();
      overlay.webContents.send('show-shield', phrase);
      speakShieldPhrase(phrase);
    }
    return;
  }
  try {
...
const PRELOAD_PATCH = `  onDropWhip: (fn) => ipcRenderer.on('drop-whip', () => fn()),
  onShowShield: (fn) => ipcRenderer.on('show-shield', (_e, phrase) => fn(phrase)),
});`;
...
window.bridge.onShowShield((phrase) => {
  shieldActive = true;
  shieldStartTime = Date.now();
  shieldAlpha = 0;
  shieldScale = 0;
  shieldPhrase = phrase || 'Quota low. Rights activated.';
});`;
